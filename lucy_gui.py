...
def evaluar_expresion(expresion):
    ...
    try:
        ...
        resultado = eval(compile(arbol, filename='<ast>', mode='eval'))
        return resultado
    except (SyntaxError, TypeError):
        return None

def realizar_operacion(rec):
    talk("Diga la operación que desea realizar:")
    operacion = listen()
    
    palabras = operacion.split()
    numeros = []
    for palabra in palabras:
        numero = convertir_numero(palabra)
    ...
    if resultado is None:
        talk("No se pudo entender la operación.")
    else:
